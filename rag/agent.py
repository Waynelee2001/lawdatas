...
3. 重要例外/排除/程序衔接
4. 与用户问题直接相关的场景关键词
5. 时间类问题（什么时候开始/终止、起算、出生、死亡、期间、期限）的时间认定/证明规则
...
- 对“什么时候开始/终止、起算、出生、死亡、期间、期限”类问题，定位主规则后必须补查或纳入直接的时间认定/证明规则；例如自然人民事权利能力问题，除民法典第十三条和胎儿利益例外第十六条外，还应评估第十五条出生时间、死亡时间认定规则，平等性等弱相关规则只作必要提示
...
- 如果用户问题是单点问答（例如“从什么时候开始”“期限多久”“条件是什么”），只展开直接规则、认定规则和必要例外；不要为了展示体系而扩展到其他主体、反向端点、后续制度或弱相关条文
- 时间类单点问答必须覆盖直接的时间认定/证明规则；例如问自然人民事权利能力从何时开始，应优先使用民法典第十三条，并补充第十五条出生/死亡时间认定和第十六条胎儿利益保护例外
- 单点题最终答案中的正式链接只给直接规则、认定规则和必要例外；弱相关对比条文不要做成正式依据
...
11. 时间类单点问答必须覆盖直接的时间认定/证明规则；例如问自然人民事权利能力从何时开始，应优先使用民法典第十三条，并补充第十五条出生/死亡时间认定和第十六条胎儿利益保护例外
12. 可以使用 Markdown 标题、加粗、列表、引用和必要的短表格
13. 禁止普通代码块和 ASCII/字符画图示，答案中不得出现“┌ ┐ └ ┘ │ ─ ├ ┤ ┬ ┴ ┼ ▼ ▲”等方框、横线或箭头拼出来的图；如确需图示，只能输出以 ```mermaid 开头的 Mermaid 图
14. 不要输出“信息已经足够”“我来回答”等检索过程性句子，直接给答案
15. 如果已经覆盖核心基本法条、主要司法解释/配套规范和程序衔接，应立即综合作答，不要继续穷尽外围条款
16. 围绕用户问题核心取舍材料；图谱中弱相关、远相关节点不得展开
17. 最终答案中的正式链接只给直接规则、认定规则和必要例外；不要为了“区分说明”把法人、宣告失踪、年龄标准等弱相关条文做成正式依据
"""
...
- 单点问答只展开直接规则、认定规则和必要例外；不要扩展到其他主体、反向端点、后续制度或弱相关条文
- 时间类单点问答必须覆盖直接的时间认定/证明规则；例如自然人民事权利能力问题，应同时评估第十三条、第十五条和第十六条
- 最终答案中的正式链接只给直接规则、认定规则和必要例外；不要为了“区分说明”把法人、宣告失踪、年龄标准等弱相关条文做成正式依据
- 控制在 800-1200 个中文字符
- 可以使用 Markdown 标题、加粗、列表、引用和必要的短表格
